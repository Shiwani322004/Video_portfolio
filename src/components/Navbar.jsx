    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
        ...
        <div className="flex items-center">
          ...
          >
            Shiwani <span className="text-emerald-500">.</span>
          </a>
        </div>
        ...
        <div className="hidden md:block">
          <a 
            ...
            className={`px-6 py-2.5 rounded-full text-sm font-black transition-all duration-500 ${
              isScrolled
                ? 'bg-gray-900 text-white hover:bg-emerald-500 hover:shadow-[0_10px_25px_rgba(16,185,129,0.25)]'
      ...
      <div 
        className={`md:hidden absolute top-full left-0 w-full transition-all duration-500 ease-in-out ${
          isOpen ? 'max-h-[460px] py-6 opacity-100 bg-zinc-950 border-b border-white/10 shadow-2xl' : 'max-h-0 opacity-0 pointer-events-none'
        }`}
      >
        <div className="flex flex-col px-6 space-y-4">
          {navLinks.map((link) => (
            <a 
              ...
              className="text-white hover:text-emerald-500 font-extrabold text-base border-b border-white/10 pb-2.5 transition-colors"
            >
              {link}
            </a>
          ))}
          <div className="pt-2">
             <a 
               href="#contact" 
               onClick={() => setIsOpen(false)} 
               className="inline-block px-6 py-3 rounded-full bg-emerald-500 text-white font-black hover:bg-emerald-400 transition-all duration-300 w-full text-center shadow-xl"
             >
               Hire Me
             </a>
          </div>
